...
function inferFields(data) {
    if (!data || data.length === 0) return { id: null, amount: null, status: null };
    ...
    headers.forEach(h => {
        cleanedToOriginalMap.set(String(h).toLowerCase().replace(/[^a-z0-9]/g, ''), h);
    });

    const findOriginalHeader = (keywords) => {
        for (const keyword of keywords) {
            // Clean the keyword before checking against the map
            const cleanedKeyword = String(keyword).toLowerCase().replace(/[^a-z0-9]/g, '');
    ...
    inferred.id = findOriginalHeader(['transaction_reference', 'transactionreference', 'transid', 'id', 'orderid', 'paymentid', 'reference', 'ref']);
    inferred.amount = findOriginalHeader(['amount', 'value', 'total', 'transactionamount', 'txamount']);
    // FIX: Ensure keywords are cleaned inside findOriginalHeader
    inferred.status = findOriginalHeader(['status', 'transactionstatus', 'txstatus', 'state', 'payment_status']);
...
function transformDataForBackend(data, inferredFields) {
    return data.map(row => {
        ...
        if (inferredFields.id && row[inferredFields.id] !== undefined) {
            newRow.transaction_reference = String(row[inferredFields.id]).trim();
        } else {
        ...
        if (inferredFields.amount && row[inferredFields.amount] !== undefined) {
            newRow.amount = parseFloat(String(row[inferredFields.amount]).trim() || 0);
        } else {
        ...
        Object.keys(row).forEach(key => {
            ...
            if (key !== inferredFields.id && key !== inferredFields.amount && key !== inferredFields.status) {
                newRow[key] = row[key];
            }
        });
        return newRow;
...
async function processFiles() {
    ...
    if (!internalFile || !providerFile) {
        showMessage("Please select both Internal and Provider CSV files.", 3000);
        return;
    }
    ...
    try {
        ...
        const transformedInternalData = transformDataForBackend(internalData, internalClientFields);
        const transformedProviderData = transformDataForBackend(providerData, providerClientFields);
        ...
        result.inferredColumns = { internal: internalClientFields, provider: providerClientFields };
        ...
        showMessage(`An error occurred: ${err.message}. Please ensure your CSVs are correctly formatted and the backend server is running on http://localhost:3000.`, 7000);
    }
}
...
function displayResults(data) {
    const { matched = [], internalOnly = [], providerOnly = [], partialMismatches = [] } = data;

    const section = document.getElementById('results');
    section.innerHTML = ''; // Clear previous results

    // Calculate total records based on reconciliation results
    const totalRecords = matched.length + internalOnly.length + providerOnly.length + partialMismatches.length;

    // Render Summary Dashboard
    renderSummaryDashboard(matched.length, internalOnly.length, providerOnly.length, partialMismatches.length, totalRecords);
    ...
    const groups = [
        { title: '✅ Matched Transactions', data: matched, colorClass: 'status-green', key: 'matched' },
        { title: '⚠️ Internal Only', data: internalOnly, colorClass: 'status-yellow', key: 'internalOnly' },
        { title: '❌ Provider Only', data: providerOnly, colorClass: 'status-red', key: 'providerOnly' },
        { title: '⚠️ Mismatched Amount/Status', data: partialMismatches, colorClass: 'status-yellow', key: 'partialMismatches' }
    ];
    ...
    groups.forEach(group => {
        ...
        const buttonsHtml = group.data.length > 0
            ? `
...
// Render Summary Dashboard
function renderSummaryDashboard(matched, internalOnly, providerOnly, partialMismatches, totalRecords) {
    const resultsSection = document.getElementById('results');
    const summaryDiv = document.createElement('div');
    summaryDiv.className = 'summary-dashboard';

    const calculatePercentage = (count) => totalRecords > 0 ? ((count / totalRecords) * 100).toFixed(1) : 0;
...
function downloadFullReport() {
    if (!lastReconciliationResult) {
        showMessage("No reconciliation results to download. Please run a reconciliation first.", 3000);
        return;
    }
...
function saveHistory(entryData) {
    const history = JSON.parse(localStorage.getItem('reconciliationHistory') || '[]');
        ...
        return `
            <div class="history-entry">
                ...
                <div class="history-actions">
                    <button onclick="viewHistoryEntry(${index})">👁️ View</button>
...
function downloadHistoryEntry(index) {
    ...
    const blob = new Blob([JSON.stringify(fullContent, null, 2)], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
